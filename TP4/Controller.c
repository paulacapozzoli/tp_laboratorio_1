...
int controller_filterHours(LinkedList* pArrayListEmployee){
    int todoOk=0;
    LinkedList* filteredList;

    filteredList=ll_filter(pArrayListEmployee,filterHours);

    mostrarEmployees(filteredList);

    if(ll_containsAll(pArrayListEmployee,filteredList)==0){
        todoOk=1;
    }
