...
static Node* getNode(LinkedList* this, int nodeIndex)
{
    ...
    if(this!=NULL&&nodeIndex>=0&&nodeIndex<ll_len(this)){
        ...
        while(nodeIndex>0){
            nodo = nodo->pNextNode;//se hace n cantidad de veces,por eso el while
            nodeIndex--;//hasta que el indice de nodo llegue a 0
        }
    }
    return nodo;
}
...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if(this!=NULL&&nodeIndex>=0&&nodeIndex<=ll_len(this)){
        ...
        if(newNode!=NULL){
            //apunto pElement del nodo al elemento pElement
            ...
            if(nodeIndex==0){
                ...
                //agrego nodo en el indice indicado
                lastNode=getNode(this, nodeIndex-1);
                newNode->pNextNode=lastNode->pElement;
                lastNode->pNextNode=newNode; //apunto el ultimo nodo al nuevo nodo
            }
            //debo actualizar el tamaño
            this->size++;
            returnAux=0;
        }
    }
...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this!=NULL&&index>=0&&index<ll_len(this)){
        curNode=getNode(this,index);

        if(curNode!=NULL){
            if(index==0){
                this->pFirstNode=curNode->pNextNode;
                this->size--;
                free(curNode);
                returnAux=0;
            }else{
                ...
                if(lastNode!=NULL){
                    lastNode->pNextNode=curNode->pNextNode;
                    this->size--;
                    free(curNode);
                    returnAux=0;
                }
...
/** \brief Crea un nuevo LinkedList filtrando segun la funcion criterio
 *
